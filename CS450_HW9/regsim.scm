    ...
    (let ((the-ops
      ...
      (define (execute)
        (let ((insts (get-contents pc))
              (breakpoint (assoc (get-contents pc) breakpoints)))
          (cond ((null? insts) 'done)
                ((and breakpoint (eq? insts (car breakpoint))) (break breakpoint))
      ...
      (define (proceed)
        (let ((insts (get-contents pc)))
          (cond ((null? insts) 'done)
                (else ((instruction-execution-proc (car insts)))
                      (execute)))))
      (define (create-breakpoint label offset)
        (let ((insts (assoc label labels)))
          (if insts
              (begin (set! insts (trim-instructions (cdr insts) offset label))
                     (if (assoc insts breakpoints)
                         (print "A breakpoint already exists at " label " with offset " offset ".")
                         (set! breakpoints (cons (list insts label offset) breakpoints))))
              (error "This machine does not contain a label " label "."))))
      (define (cancel-breakpoint label offset)
        (let ((insts (assoc label labels)))
          (define (loop breaks)
            (cond ((null? breaks) (error "No breakpoint at label " label " with offset " offset "."))
                  ((eq? insts (caar breaks)) (print "Breakpoint at " label " with offset " offset " cancelled.")
                                             (cdr breaks))
                  (else (cons (car breaks) (loop (cdr breaks))))))
          (if insts
              (begin (set! insts (trim-instructions (cdr insts) offset label))
                     (set! breakpoints (loop breakpoints)))
              (error "This machine does not contain the label " label "."))))
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'create-breakpoint) create-breakpoint)
              ((eq? message 'cancel-breakpoint) cancel-breakpoint)
              ((eq? message 'cancel-all-breakpoints) (set! breakpoints '())
                                                     (print "Cancelled all breakpoints."))
...
; continue machine processing
(define (proceed-machine machine)
  (machine 'proceed))

; create a breakpoint offset from label by n
(define (set-breakpoint machine label n)
  ((machine 'create-breakpoint) label n))

; cancel a breakpoint offset from label by n
(define (cancel-breakpoint machine label n)
  ((machine 'cancel-breakpoint) label n))

; cancel all breakpoints in a machine
(define (cancel-all-breakpoints machine)
  (machine 'cancel-all-breakpoints))

; given a set of instructions and an offset n trims the instructions until the offset is 1
; throws an error if the offset is less than 1 or if the offset is out of bounds of the instruction set
(define (trim-instructions insts n label)
  (define (loop insts n)
    (if (= n 1)
        insts
        (loop (cdr insts) (- n 1))))
  (cond ((< n 1) (error "Expected breakpoint offset >= 1 but got " n "."))
        ((< (length insts) n) (error "Label + offset out of bounds of machine instructions. " label n))
        (else (loop insts n))))

; prints breakpoint information and halts execution
(define (break breakpoint)
  (print "Stopped at " (breakpoint-label breakpoint) " with offset " (breakpoint-offset breakpoint) "."))

; prints a list of arguments and then a newline
(define (print . args)
  (define (loop rem)
    (if (null? rem)
        (newline)
        (begin (display (car rem))
               (loop (cdr rem)))))
  (loop args))

; retrieves the instructions of a breakpoint
(define (breakpoint-instructions breakpoint) (car breakpoint))

; retrieves the label of a breakpoint
(define (breakpoint-label breakpoint) (cadr breakpoint))

; retrieves the offset of a breakpoint
...
(define gcd-registers '(a b t))

(define gcd-ops (list
                 (list 'rem remainder)
                 (list '= =)))

(define gcd-text '(test-b
...
(define gcd-machine (make-machine gcd-registers gcd-ops gcd-text))

;(define (solve-gcd a b)
;  (set-register-contents! gcd-machine 'a a)
;  (set-register-contents! gcd-machine 'b b)
;  (if (eq? (start gcd-machine) 'break)
;      'break
;      (get-register-contents gcd-machine 'a)))



;(define test (set-breakpoint gcd-machine 'test-b 1))